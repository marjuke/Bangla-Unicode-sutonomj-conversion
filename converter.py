...
class Unicode:
	...
	def reArrangeUnicodeConvertedText(self, str):
		...
		while i < util.mb_strlen(str):
			...
			if (i < (util.mb_strlen(str) - 1) and util.mbCharAt(str, i) == 'র' and self.IsBanglaHalant(
                    util.mbCharAt(str, i + 1)) and not self.IsBanglaHalant(util.mbCharAt(str, i - 1))):
				j = 1
				while (True):
					if (i - j < 0):
						break

					if (self.IsBanglaBanjonborno(util.mbCharAt(str, i - j)) and self.IsBanglaHalant(
                            util.mbCharAt(str, i - j - 1))):
						j += 2
		...
		while i< util.mb_strlen(str):
			if (i < util.mb_strlen(str) - 1 and util.mbCharAt(str, i) == 'র' and self.IsBanglaHalant(
                    util.mbCharAt(str, i + 1)) and not self.IsBanglaHalant(util.mbCharAt(str, i - 1)) and self.IsBanglaHalant(
                    util.mbCharAt(str, i + 2))):
				j = 1
				while (True):
					if (i - j < 0):
						break
			...
			if (i > 0 and util.mbCharAt(str, i) == '\u09CD' and (self.IsBanglaKar(util.mbCharAt(str, i - 1)) or self.IsBanglaNukta(
                    util.mbCharAt(str, i - 1))) and i < util.mb_strlen(str) - 1):
			...
			if (i > 0 and i < util.mb_strlen(str) - 1 and util.mbCharAt(str, i) == '\u09CD' and util.mbCharAt(str, i - 1) == '\u09B0' and util.mbCharAt(str, i - 2) != '\u09CD' and self.IsBanglaKar(
                    util.mbCharAt(str, i + 1))):
				temp = util.subString(str, 0, i - 1)
				temp += util.mbCharAt(str, i + 1)
				temp += util.mbCharAt(str, i - 1)
				temp += util.mbCharAt(str, i)
			...
			if (i < util.mb_strlen(str) - 1 and self.IsBanglaNukta(util.mbCharAt(str, i)) and self.IsBanglaPostKar(
                    util.mbCharAt(str, i + 1))):
				temp = util.subString(str, 0, i)
				temp += util.mbCharAt(str, i + 1)
				temp += util.mbCharAt(str, i)
	...
	def reArranceUnicodeTextForASCI(self, str):
		...
		while i< util.mb_strlen(str):
			...
			if i<(util.mb_strlen(str) - 1) and self.IsBanglaHalant(util.mbCharAt(str, i)) and util.mbCharAt(str, i - 1)== 'র' and not self.IsBanglaHalant(
                    util.mbCharAt(str, i - 2)):
				j=1
				aZ=0
				...
				while True:
					...
					elif self.IsBanglaBanjonborno(util.mbCharAt(str, i + j)) and self.IsBanglaPreKar(
                            util.mbCharAt(str, i + j + 1)):
						aZ=1
						break
